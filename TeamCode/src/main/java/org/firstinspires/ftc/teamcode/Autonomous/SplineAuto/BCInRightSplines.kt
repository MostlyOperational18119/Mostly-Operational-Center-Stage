...
class BCInRightSplines : AutoBoilerplate() {
    ...
    override val startingPose: Pose2d = Pose2d(15.01, 62.69, Math.toRadians(90.00))
        ...
        return when (detection) {
            Variables.Detection.LEFT -> drive!!.trajectorySequenceBuilder(startingPose)
                    ...
                    .splineToLinearHeading(
                        Pose2d(55.0, 45.07, Math.toRadians(180.00)),
                        Math.toRadians(0.0)
                    )
                    .waitSeconds(.1)
            ...
            Variables.Detection.CENTER -> drive!!.trajectorySequenceBuilder(startingPose)
                        ...
                        .waitSeconds(.5)
                        .splineToLinearHeading(
                            Pose2d(55.0, 39.46, Math.toRadians(180.00)),
                            Math.toRadians(0.00)
                        )
                        .waitSeconds(.1)
                        .addTemporalMarker { autoServo!!.position = servoPlacePosition }
